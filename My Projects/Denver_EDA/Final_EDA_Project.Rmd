...
## Interpretation of Median Listing Size by Neighborhood
...
avg_size_tbl <- Denver_clean1 |>dplyr::filter(neighbourhood_cleansed %in% neigh) |>
  dplyr::group_by(neighbourhood_cleansed) |>dplyr::summarise("Size" = median(bedrooms, na.rm = TRUE),
                                                             .groups = "drop") |>
...
```{r,echo=FALSE, message=FALSE, warning=FALSE}
knitr::kable(avg_size_tbl,digits = 2,caption = "Median listing bedrooms (Size) by neighborhood") |> kableExtra::kable_styling(full_width = FALSE)
```
